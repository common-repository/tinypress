...
(function ($, window, document, pluginObject) {
    ...
    $(document).on('click', '.tiny-slug-wrap .tiny-slug-preview', function () {
        ...
        el_input.value = el_preview.text().replace(' ', '');
        el_input.select();
...
function wpFeSanitizeTitle(title) {
    ...
    return replaceSpacesWithDash(
        removeHTMLEntities(
            ...
            .replace(/[\/\.]/g, '-')
            // Replace anything that is not a: word character space nor a dash (-) with an empty string (i.e. remove it).
            .replace(/[^\w\s-]+/g, '')
    );
